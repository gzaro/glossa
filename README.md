...
- [1. Δομή προγράμματος](#1-δομή-προγράμματος)
- [2. Τύποι δεδομένων](#2-τύποι-δεδομένων)
  - [2.1 Ακέραιος](#21-ακέραιος)
  - [2.2 Πραγματικός](#22-πραγματικός)
  - [2.3 Χαρακτήρας](#23-χαρακτήρας)
  - [2.4 Λογικός](#24-λογικός)
- [3. Τελεστές](#3-τελεστές)
  - [3.1 Αριθμητικοί](#31-αριθμητικοί)
  - [3.2 Συγκριτικοί](#32-συγκριτικοί)
  - [3.3 Λογικοί](#33-λογικοί)
- [4. Αριθμητικές συναρτήσεις](#4-αριθμητικές-συναρτήσεις)
- [5. Εντολές επιλογής](#5-εντολές-επιλογής)
  - [5.1 ΑΝ](#51-αν)
  - [5.2 ΕΠΙΛΕΞΕ](#52-επιλεξε)
- [6. Εντολές επανάληψης](#6-εντολές-επανάληψης)
  - [6.1 ΟΣΟ... ΕΠΑΝΑΛΑΒΕ](#61-οσοεπαναλαβε)
  - [6.2 ΜΕΧΡΙΣ_ΟΤΟΥ](#62-μεχριςοτου)
  - [6.3 ΓΙΑ... ΑΠΟ... ΜΕΧΡΙ](#63-για-απομεχρι)
- [7. Πίνακες](#7-πίνακες)
  - [7.1 Μονοδιάστατοι](#71-μονοδιάστατοι)
...
Ο τύπος αυτός περιλαμβάνει τους πραγματικούς αριθμούς που γνωρίζουμε από τα μαθηματικά. Οι αριθμοί 3.14159, 2.71828, -112.45, 0.45 είναι πραγματικοί αριθμοί. Και οι πραγματικοί αριθμοί μπορούν να είναι θετικοί, αρνητικοί ή μηδέν.
...
6.1 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο εμφανίζει την προπαίδεια:
...
6.2 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο δέχεται έναν ακέραιο αριθμό από το πληκτρολόγιο και τον εμφανίζει σε δυαδική μορφή (κάθετα)

6.3 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο δέχεται έναν ακέραιο αριθμό σε δυαδική μορφή από το πληκτρολόγιο και τον εμφανίζει σε δεκαδική μορφή

6.4 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο δέχεται έναν ακέραιο αριθμό από το πληκτρολόγιο και ελέγχει αν είναι παλίνδρομος. Δηλαδή αν διαβάζεται το ίδιο και από τις 2 κατευθύνσεις π.χ. 1, 232, 1221.
...
7.1.1 Γράψε ένα πρόγραμμα που αποθηκεύει 10 αριθμούς που δέχεται από το πληκτρολόγιο, υπολόγισε τον μέσο όρο και εμφάνισε τους αριθμούς που είναι μικρότεροι από τον μέσο όρο.

7.1.2 Γράψε ένα πρόγραμμα που αποθηκεύει 10 αριθμούς που δέχεται από το πληκτρολόγιο, του αποθηκεύει σε έναν πίνακα, τον ταξινομεί με αύξουσα σειρά και τον εμφανίζει ταξινομημένο.

7.1.3 Γράψε ένα πρόγραμμα που δέχεται μια λέξη από το πληκτρολόγιο και εμφανίζει τις θέσεις που βρίσκεται αυτή η λέξη, στον παρακάτω πίνακα. Δοκίμασε να δώσεις μια λέξη που υπάρχει, με κεφαλαία, μικρά, με ή χωρίς τόνο. Τι παρατηρείς και γιατί συμβαίνει αυτό;

~~~
ΠΡΟΓΡΑΜΜΑ ΑναζήτησηΛέξεων

ΜΕΤΑΒΛΗΤΕΣ
	ΧΑΡΑΚΤΗΡΕΣ: πίνακας[10]
	
 
ΑΡΧΗ

	πίνακας[1] <- 'πρόγραμμα'
	πίνακας[2] <- 'επιστημονικών'
	πίνακας[3] <- 'πρόσκληση'
	πίνακας[4] <- 'πρόσκληση'
	πίνακας[5] <- 'ταυτότητα'
	πίνακας[6] <- 'πρόγραμμα'
	πίνακας[7] <- 'συμμετοχή'
	πίνακας[8] <- 'συνδυασμούς'
	πίνακας[9] <- 'συμμετοχή'
	πίνακας[10] <- 'πρόγραμμα'
...
7.1.4 Γράψε ένα πρόγραμμα που εμφανίζει τις λέξεις που υπάρχουν περισσότερες από μια φορές, στον παρακάτω πίνακα.

~~~
ΠΡΟΓΡΑΜΜΑ ΠίνακαςΛέξεων
...
7.1.5 Γράψε ένα πρόγραμμα που δέχεται 10 ακεραίους από το πληκτρολόγιο, τους αποθηκεύει σε έναν πίνακα και μετά βρίσκει και εμφανίζει τον δεύτερο μεγαλύτερο από αυτούς.

7.1.6 Συμπλήρωσε το παρακάτω πρόγραμμα, ώστε στο τέλος, όλες οι μηδενικές τιμές, να μεταφερθούν στο τέλος του πίνακα. Δηλαδή στις θέσεις 7 έως 10. Οι μη μηδενικές τιμές να καλύπτουν τις θέσεις 1 έως 6.

~~~
ΠΡΟΓΡΑΜΜΑ ΜηδένΣτοΤέλος

ΜΕΤΑΒΛΗΤΕΣ
	ΑΚΕΡΑΙΕΣ: πίνακας[10], i
...
ΑΡΧΗ
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! ΓΡΑΨΕ ΕΔΩ ΤΟΝ ΚΩΔΙΚΑ ΠΟΥ     !
  ! ΜΕΤΑΦΕΡΕΙ ΤΑ ΜΗΔΕΝ ΣΤΟ ΤΕΛΟΣ ! 
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
7.1.7 Γράψε ένα πρόγραμμα που δέχεται έναν ακέραιο από το πληκτρολόγιο και εμφανίζει τη θέση που βρίσκεται στον παρακάτω πίνακα.

~~~
ΠΡΟΓΡΑΜΜΑ ΑναζήτησηΑκεραίων
...
7.1.8 Τρέξε το προηγούμενο πρόγραμμα που έφτιαξες, δίνοντας τον αριθμό 81. Πόσους ελέγχους ισότητας εκτέλεσε μέχρι να βρει τη θέση 7; Μπορείς να φτιάξεις ένα πρόγραμμα που θα βρίσκει τη θέση του 81 με 2 ελέγχους;

7.1.9 Συμπλήρωσε το παρακάτω πρόγραμμα, ώστε να μεταφέρει όλες τις τιμές από τους πίνακας1 και πίνακας2, οι οποίοι είναι ταξινομημένοι, στον πίνακας3, έτσι ώστε και ο πίνακας3 να είναι ταξινομημένος.
...
7.1.10 Γράψε ένα πρόγραμμα που εμφανίζει όλες τις τριψήφιες λέξεις που μπορούν να δημιουργηθούν από τα γράμματα Α, Β, Γ. Δηλαδή να εμφανίζει:
...
8.1 Γράψε ένα πρόγραμμα που δέχεται δύο ακεραίους n και k και υπολογίζει τον [διωνυμικό συντελεστή](https://el.wikipedia.org/wiki/%CE%94%CE%B9%CF%89%CE%BD%CF%85%CE%BC%CE%B9%CE%BA%CF%8C%CF%82_%CF%83%CF%85%CE%BD%CF%84%CE%B5%CE%BB%CE%B5%CF%83%CF%84%CE%AE%CF%82) n! / (k! * (n - k)!)