...
- [3. Τελεστές](#operators)
  - [3.1 Αριθμητικοί τελεστές](#numeric-operators)
  - [3.2 Συγκριτικοί τελεστές](#comparison-operators)
  - [3.3 Λογικοί τελεστές](#logigal-operators)
- [4. Αριθμητικές συναρτήσεις](#numeric-functions)
- [5. Εντολές επιλογής](#selection)
  - [5.1 ΑΝ](#if)
  - [5.2 ΕΠΙΛΕΞΕ](#case)
- [7. Εντολές επανάληψης](#loops)
- [8. Πίνακες](#arrays)
  - [8.1 Μονοδιάστατοι](#81-μονοδιάστατοι)
...
### 3. Αριθμητικοί τελεστές και αριθμητικές συναρτήσεις {#numeric-operators-functions}
...
### 4. Συγκριτικοί τελεστές {#comparison-operators}
...
### 5. Λογικοί τελεστές {#logigal-operators}
...
7.3.1 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο εμφανίζει την προπαίδεια:
...
7.3.2 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο δέχεται έναν ακέραιο αριθμό από το πληκτρολόγιο και τον εμφανίζει σε δυαδική μορφή (κάθετα)

7.3.3 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο δέχεται έναν ακέραιο αριθμό σε δυαδική μορφή από το πληκτρολόγιο και τον εμφανίζει σε δεκαδική μορφή

7.3.4 Γράψε ένα πρόγραμμα σε γλώσσα, το οποίο δέχεται έναν ακέραιο αριθμό από το πληκτρολόγιο και ελέγχει αν είναι παλίνδρομος. Δηλαδή αν διαβάζεται το ίδιο και από τις 2 κατευθύνσεις π.χ. 1, 232, 1221.
...
8.1.1 Γράψε ένα πρόγραμμα που αποθηκεύει 10 αριθμούς που δέχεται από το πληκτρολόγιο, υπολόγισε τον μέσο όρο και εμφάνισε τους αριθμούς που είναι μικρότεροι από τον μέσο όρο.

8.1.2 Γράψε ένα πρόγραμμα που αποθηκεύει 10 αριθμούς που δέχεται από το πληκτρολόγιο, του αποθηκεύει σε έναν πίνακα, τον ταξινομεί με αύξουσα σειρά και τον εμφανίζει ταξινομημένο.

8.1.3 Γράψε ένα πρόγραμμα που δέχεται μια λέξη από το πληκτρολόγιο και εμφανίζει τις θέσεις που βρίσκεται αυτή η λέξη, στον παρακάτω πίνακα. Δοκίμασε να δώσεις μια λέξη που υπάρχει, με κεφαλαία, μικρά, με ή χωρίς τόνο. Τι παρατηρείς και γιατί συμβαίνει αυτό;
...
8.1.4 Γράψε ένα πρόγραμμα που εμφανίζει τις λέξεις που υπάρχουν περισσότερες από μια φορές, στον παρακάτω πίνακα.
...
8.1.5 Γράψε ένα πρόγραμμα που δέχεται έναν ακέραιο από το πληκτρολόγιο και εμφανίζει τη θέση που βρίσκεται στον παρακάτω πίνακα.
...
8.1.6 Τρέξε το προηγούμενο πρόγραμμα που έφτιαξες, δίνοντας τον αριθμό 81. Πόσους ελέγχους ισότητας εκτέλεσε μέχρι να βρει τη θέση 7; Μπορείς να φτιάξεις ένα πρόγραμμα που θα βρίσκει τη θέση του 81 με 2 ελέγχους;

8.1.7 Γράψε ένα πρόγραμμα που μεταφέρει όλες τις τιμές από τους πίνακας1 και πίνακας2, οι οποίοι είναι ταξινομημένοι, στον πίνακας3, έτσι ώστε και ο πίνακας3 να είναι ταξινομημένος.
...
8.1.8 Γράψε ένα πρόγραμμα που εμφανίζει όλες τις τριψήφιες λέξεις που μπορούν να δημιουργηθούν από τα γράμματα Α, Β, Γ. Δηλαδή να εμφανίζει:
...
9.1 Γράψε ένα πρόγραμμα που δέχεται δύο ακεραίους n και k και υπολογίζει τον [διωνυμικό συντελεστή](https://el.wikipedia.org/wiki/%CE%94%CE%B9%CF%89%CE%BD%CF%85%CE%BC%CE%B9%CE%BA%CF%8C%CF%82_%CF%83%CF%85%CE%BD%CF%84%CE%B5%CE%BB%CE%B5%CF%83%CF%84%CE%AE%CF%82) n! / (k! * (n - k)!)